...
export default function Page() {
  ...
  const enqueueJob = (promptId?: number) => setJobs((prev) => [...prev, { id: Date.now(), status: "queued", promptId }]);
  const advanceJob = (id: number) =>
    setJobs((prev) => prev.map((j) => (j.id === id ? { ...j, status: j.status === "queued" ? "running" : "done" } : j)));
  const addPrompt = (name: string, text: string) => setPrompts((prev) => [...prev, { id: Date.now(), name, text }]);
  return (
    <div className="space-y-6">
      ...
      <section className="grid md:grid-cols-3 gap-4">
        <div className="border rounded p-4">
          ...
          <div className="text-sm mb-2">Queued: {jobs.filter(j=>j.status==="queued").length} • Running: {jobs.filter(j=>j.status==="running").length} • Done: {jobs.filter(j=>j.status==="done").length}</div>
          <button className="border px-2 py-1 rounded" onClick={()=>enqueueJob()}>Enqueue Job</button>
          <ul className="text-sm list-disc pl-5 mt-2">
            {visible.map((j)=>(
              <li key={j.id} className="flex items-center justify-between">
                <span>Job #{j.id} — {j.status}</span>
                {j.status!=="done" && (
                  <button className="text-blue-600" onClick={()=>advanceJob(j.id)}>Advance</button>
                )}
              </li>
            ))}
          </ul>
        ...
        <div className="border rounded p-4">
              ...
              .map((p)=>(
                <li key={p.id} className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button className="border px-2 py-1 rounded" onClick={()=>enqueueJob(p.id)}>Run</button>
                  </div>
                </li>
              ))}
          </ul>
          <div className="mt-3 flex gap-2">
            ...
            <button className="bg-black text-white px-3 py-1 rounded" onClick={()=>{
              const n = (document.getElementById("pname") as HTMLInputElement)?.value || "Untitled";
      ...
      <section className="border rounded p-4">
        ...
        <div className="flex gap-2">
          <input className="border px-2 py-1" placeholder="Search prompts" value={q} onChange={(e)=>setQ(e.target.value)} />
      ...
      <section className="border rounded p-4">
        ...
        <form action="https://formspree.io/f/ai" method="POST" className="space-y-2">
          ...
          <textarea name="message" className="border px-2 py-1 w-full" placeholder="What workflows to automate?" />
