  ...
  return (
    <div className='space-y-6'>
      ...
      <p className='text-sm text-gray-600'>
        Mock login, RBAC, customers CRUD, metrics, notifications.
      </p>
          ...
          {user ? (
            ...
            <form className='space-y-2' onSubmit={e => e.preventDefault()}>
              ...
              <button
                type='button'
                onClick={() =>
                  signIn(email || 'user@example.com', (name || 'support') as 'admin' | 'support')
                }
          ...
          <ul className='text-sm list-disc pl-5'>
            {customers.map(c => (
              <li key={c.id} className='flex items-center justify-between'>
                ...
                {canEdit && (
                  <button onClick={() => removeCustomer(c.id)} className='text-red-600'>
                    Delete
                  </button>
                )}
              </li>
          ...
          {canEdit ? (
            <div className='mt-3 flex gap-2'>
              ...
              <input
                ...
                onChange={e => setEmail(e.target.value)}
              />
              <button onClick={addCustomer} className='bg-black text-white px-3 py-1 rounded'>
                Add
              </button>
            </div>
          ) : (
