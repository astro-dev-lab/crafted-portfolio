...
export default function Page() {
  ...
  const sendMessage = () => {
    if (!user) return;
    if (!text) return;
    setMessages((prev) => [...prev, { id: Date.now(), from: user.role === "provider" ? "provider" : "patient", text }]);
    setText("");
  };
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold">Patient Portal Simulation</h1>
      <p className="text-sm text-gray-600">Mock auth roles, messaging, appointments, lab results, notifications.</p>
        ...
        <div className="border rounded p-4">
          ...
          {user ? (
            ...
            <div className="space-y-2">
              <input className="border px-2 py-1 w-full" placeholder="email" value={email} onChange={(e)=>setEmail(e.target.value)} />
              <select className="border px-2 py-1 w-full" value={role} onChange={(e)=>setRole(e.target.value)}>
                <option value="patient">patient</option>
                <option value="provider">provider</option>
              </select>
              <button className="bg-black text-white px-3 py-1 rounded" onClick={()=>signIn(email||"patient@example.com", role as any)}>Sign In (mock)</button>
            </div>
          )}
        </div>
        <div className="border rounded p-4">
          ...
          <ul className="text-sm list-disc pl-5">
            {messages.map((m)=>(
              <li key={m.id}><span className="font-medium">{m.from}:</span> {m.text}</li>
            ))}
          </ul>
          <div className="mt-3 flex gap-2">
            <input className="border px-2 py-1 w-full" placeholder="Type a message" value={text} onChange={(e)=>setText(e.target.value)} />
            <button className="bg-black text-white px-3 py-1 rounded" onClick={sendMessage}>Send</button>
          </div>
        </div>
        <div className="border rounded p-4">
      ...
      <section className="border rounded p-4">
        ...
        <label className="block text-sm"><input type="checkbox" className="mr-2" /> Email alerts</label>
        <label className="block text-sm"><input type="checkbox" className="mr-2" /> SMS reminders</label>
      </section>
      ...
      <section className="border rounded p-4">
        ...
        <form action="https://formspree.io/f/health" method="POST" className="space-y-2">
          ...
          <textarea name="message" className="border px-2 py-1 w-full" placeholder="HIPAA needs & workflows" />
