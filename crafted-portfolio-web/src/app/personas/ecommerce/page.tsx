...
export default function Page() {
  ...
  const addProduct = () => {
    ...
    setProducts(prev => [...prev, { id: Date.now(), name: newName, price, category: 'Gadgets' }]);
    setNewName('');
    setNewPrice('');
  };
  return (
      ...
      <section className='grid md:grid-cols-3 gap-4'>
        <div className='border rounded p-4'>
            ...
            {filtered.map(p => (
              <li key={p.id} className='flex items-center justify-between'>
                ...
                <div className='flex gap-2'>
                  <button onClick={() => addToCart(p.id)} className='border px-2 py-1 rounded'>
                    Add to cart
                  </button>
          ...
          <div className='text-xs text-gray-500 mt-2'>
            Cart: {cart.length} items • Wishlist: {wishlist.length}
          </div>
        </div>
        <div className='border rounded p-4'>
      ...
      <section className='border rounded p-4'>
        ...
        <form action='https://formspree.io/f/commerce' method='POST' className='space-y-2'>
          ...
          <textarea
            name='message'
            className='border px-2 py-1 w-full'
            placeholder='Scale & integrations needed'
          />
