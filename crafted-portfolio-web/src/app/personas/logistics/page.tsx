...
export default function Page() {
  ...
  const [inventory, setInventory] = useState<{ sku: string; qty: number }[]>([
    { sku: "SKU-001", qty: 124 },
    { sku: "SKU-002", qty: 87 },
  ]);
  const [orders, setOrders] = useState<string[]>(["Received", "Picking", "In-transit"]);
  const [drivers, setDrivers] = useState<string[]>(["Driver A — Van 12", "Driver B — Truck 7"]);

  const dispatch = () => setOrders((prev)=>[...prev, "Delivered"]);
  const move = () => setPositions((prev)=> prev.map(p=> ({...p, lat: p.lat + (Math.random()-0.5)*0.01, lng: p.lng + (Math.random()-0.5)*0.01 })));
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold">Logistics Command Center</h1>
      <p className="text-sm text-gray-600">Live map mock, inventory, order flow, drivers, dispatch.</p>
      ...
      <section className="grid md:grid-cols-3 gap-4">
        <div className="border rounded p-4">
          ...
          <ul className="text-sm list-disc pl-5 mt-2">
            {positions.map(p=> (
              <li key={p.id}>{p.id}: {p.lat.toFixed(4)}, {p.lng.toFixed(4)}</li>
            ))}
          </ul>
          <button className="mt-2 border px-2 py-1 rounded" onClick={move}>Simulate movement</button>
        </div>
        <div className="border rounded p-4">
          <h2 className="font-medium mb-2">Inventory Dashboard</h2>
          <div className="grid grid-cols-2 gap-3 text-sm">
            {inventory.map(i=> (
              <div key={i.sku} className="bg-gray-100 p-3 rounded">{i.sku} — {i.qty}</div>
            ))}
          </div>
        </div>
        <div className="border rounded p-4">
      ...
      <section className="grid md:grid-cols-2 gap-4">
        ...
        <div className="border rounded p-4">
          ...
          <button className="bg-black text-white px-3 py-1 rounded" onClick={dispatch}>Dispatch (mock)</button>
        </div>
      </section>
      ...
      <section className="border rounded p-4">
        ...
        <form action="https://formspree.io/f/logistics" method="POST" className="space-y-2">
          ...
          <textarea name="message" className="border px-2 py-1 w-full" placeholder="Data flows & tracking needs" />
